...
def get_wallet_balance():
    """ดึงยอด PALM token balance จาก wallet โดยใช้ Private Key แบบบ้าน B"""
    try:
        # ใช้ Private Key เพื่อสร้าง account object และดึง address
        account = web3.eth.account.from_key(PRIVATE_KEY)

        # สร้าง ABI สำหรับ balanceOf function (แบบบ้าน B)
        balance_abi = [
            {
                "constant": True,
...
def get_transaction_history():
    ...
    try:
        ...
        cur.execute(
            """
            SELECT total_generated, delta_generated, ts 
            FROM energy_log 
            ORDER BY id DESC 
            LIMIT 10
        """
            ...
            history.append(
                {
                    ...
                    "delta_generated": row[1],
                    "timestamp": row[2],
                    "type": "Energy Sale" if row[1] > 0 else "No Sale",
                }
            )
        return history
    except Exception as e:
...
def get_monthly_summary():
    ...
    try:
        ...
        cur.execute(
            """
            SELECT 
                SUM(delta_generated) as total_delta,
...
<head>
    ...
    <title>🏠 House A - Energy Dashboard</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        <div class="header">
            <h1>🏠 House A Energy Dashboard</h1>
            <div class="role-badge">SELL ONLY - Energy Producer</div>
        </div>
        
        <div class="stats-grid" id="statsGrid">
            <div class="stat-card">
                <div class="stat-icon">⚡</div>
                <div class="stat-title">ไฟที่ขายได้ (Delta)</div>
                <div class="stat-value" id="deltaGenerated">0<span class="stat-unit">หน่วย</span></div>
            </div>
            
            <div class="stat-card">
                <div class="stat-icon">🔋</div>
                <div class="stat-title">ไฟรวมที่ผลิต (Total)</div>
                <div class="stat-value" id="totalGenerated">0.00000<span class="stat-unit">kWh</span></div>
            </div>
        ...
        <div class="history-section">
            <h2 class="history-title">📊 ประวัติการขายพลังงาน</h2>
            <table class="history-table">
                <thead>
                    <tr>
                        <th>เวลา</th>
                        <th>ไฟที่ขาย (หน่วย)</th>
                        <th>ไฟรวม (kWh)</th>
                        <th>ประเภท</th>
                    </tr>
                </thead>
    ...
    <script>
        ...
        function initializeCountdown() {
            const savedCountdownStart = localStorage.getItem('houseA_countdownStart');
            const now = Date.now();
            ...
            if (savedCountdownStart) {
                ...
                localStorage.setItem('houseA_countdownStart', countdownStartTime.toString());
            }
        }
        ...
        async function fetchData() {
            try {
                ...
                // Update stats - คูณ delta_generated ด้วย 1000 สำหรับการแสดงผล
                document.getElementById('deltaGenerated').innerHTML = 
                    `${Math.round(data.delta_generated * 1000)}<span class="stat-unit">หน่วย</span>`;
                document.getElementById('totalGenerated').innerHTML = 
                    `${data.total_generated.toFixed(5)}<span class="stat-unit">kWh</span>`;
        ...
        function updateHistoryTable(history) {
            ...
            tbody.innerHTML = history.map(record => `
                <tr>
                    ...
                    <td>${Math.round(record.delta_generated * 1000)}</td>
                    <td>${record.total_generated.toFixed(5)}</td>
                    <td class="${record.delta_generated > 0 ? 'energy-sale' : 'no-sale'}">
                        ${record.type}
                    </td>
                </tr>
            `).join('');
...
def update_current_data(total_gen, delta_gen):
...
flask_thread.start()
print("🌐 Dashboard เริ่มทำงานที่ http://localhost:5000")
...
try:
    while True:
        ...
        update_current_data(total_gen, delta_gen)
